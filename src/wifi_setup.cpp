...
// --- esp_event callbacks ---
//
// esp_wifi_config publishes on the default event loop under WIFI_CFG_EVENT,
// so these run on the system event task and share it with IDF's own
// WIFI_EVENT / IP_EVENT handlers. Keep them short.
...
httpd_handle_t wifi_setup_init() {
    ...
    // Initialize esp_wifi_config. Provisioning is the SoftAP captive portal
    // (enable_ap below). Improv, the library Web UI and Network Provisioning
    // over BLE are all off — see sdkconfig.defaults for why BLE and the
    // SoftAP cannot both run. RepelBridge serves its own control UI from
    // LittleFS.
    //
    // WIFI_CFG_DEFAULTS is mandatory: wifi_cfg_init() does not patch unset
    // fields, so a field this struct does not mention is taken literally
    // (retry_interval_ms == 0 is rejected outright, auto_reconnect == false
    // silently never reconnects). The overrides are assignments rather than
    // further designated initialisers because C++ requires designators in
    // declaration order, and the macro already sets the last three members.
